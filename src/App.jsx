...
function App() {
  ...
  // Handle search from PropertyHero
  const handleHeroSearch = (searchQuery, searchType) => {
    setHasSearched(true);
    let results = [...propertiesData.properties];
    ...
    if (searchQuery && searchQuery.trim() !== '') {
      ...
      results = results.filter(prop => {
        const location = prop.location.toUpperCase();
  ...
  const handleSearch = (criteria) => {
    ...
    // Filter by price range
    if (criteria.minPrice && criteria.minPrice !== '') {
      results = results.filter(prop => prop.price >= Number(criteria.minPrice));
    }
    if (criteria.maxPrice && criteria.maxPrice !== '') {
      results = results.filter(prop => prop.price <= Number(criteria.maxPrice));
    }

    // Filter by bedrooms
    if (criteria.minBedrooms && criteria.minBedrooms !== '') {
      results = results.filter(prop => prop.bedrooms >= Number(criteria.minBedrooms));
    }
    if (criteria.maxBedrooms && criteria.maxBedrooms !== '') {
      results = results.filter(prop => prop.bedrooms <= Number(criteria.maxBedrooms));
    }
    ...
    if (criteria.postcode && criteria.postcode.trim() !== '') {
      ...
      results = results.filter(prop => {
        const location = prop.location.toUpperCase();
        return location.includes(searchPostcode);
      });
    }
    ...
    <DndProvider backend={HTML5Backend}>
      <Router>
        <div className="App">
          ...
          <main className="main-content">
            <Routes>
              {/* Home Page with PropertyHero - Route: / */}
              <Route 
                path="/" 
                element={
                  <PropertyHero onSearch={handleHeroSearch} />
              ...
              {/* Search Property Page - Route: /search */}
              <Route 
                path="/search" 
                element={
                  <>
